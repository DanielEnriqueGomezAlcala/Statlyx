...
def static_chart_bars_breakdown_resume(df, titulo_grafica):
    ...
    fig.update_layout(
        ...
        barmode='group',
        plot_bgcolor='white',
        legend=dict(orientation="v", yanchor="top", y=-2, xanchor="center", x=0.5, font=dict(size=20)),
        margin=dict(r=50, t=100, l=100, b=100),
    )

    fig.update_xaxes(
        tickangle=-45, showgrid=False, linecolor='black',
        ticks='outside', tickfont=dict(size=20),
        type='category', categoryorder='array', categoryarray=list(etiquetas_x.values),
    )

    fig.update_yaxes(
        range=[0, 115], showgrid=True, gridcolor='lightgray',
        linecolor='black', ticksuffix="%", tickformat=".1f", tickfont=dict(size=20),
    )
