...
def static_chart_bars_itineraries_breakdown(df, titulo_grafica): 
    ...
    etiquetas_x = df_medias['Itinerario']
    ...
    if 'Exito' in df_medias.columns:
        fig.add_trace(go.Bar(
            ...
            hovertemplate='Itinerario: %{x}<br>Éxito: %{y:.2f}%<extra></extra>'
        ))
    ...
    if 'Rendimiento' in df_medias.columns:
        fig.add_trace(go.Bar(
            ...
            textposition='outside',
            hovertemplate='Itinerario: %{x}<br>Rendimiento: %{y:.2f}%<extra></extra>'
        ))
