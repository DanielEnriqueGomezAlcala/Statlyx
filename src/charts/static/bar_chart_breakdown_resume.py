...
def static_chart_bars_breakdown_resume(df, titulo_grafica): 
    df_medias = df.pivot_table(
        index=['Curso', 'Cuatrimestre'],
        columns='Tasa',
        values='Valor',
        aggfunc='mean'
    ).reset_index()

    etiquetas_x = "C" + df_medias['Curso'].astype(str) + " - Cuat. " + df_medias['Cuatrimestre'].astype(str)
    ...
    if 'Exito' in df_medias.columns:
        fig.add_trace(go.Bar(
            ...
            hovertemplate='Periodo: %{x}<br>Éxito: %{y:.2f}%<extra></extra>'
        ))
    ...
    if 'Rendimiento' in df_medias.columns:
        fig.add_trace(go.Bar(
            ...
            textposition='outside',
            hovertemplate='Periodo: %{x}<br>Rendimiento: %{y:.2f}%<extra></extra>'
        ))
