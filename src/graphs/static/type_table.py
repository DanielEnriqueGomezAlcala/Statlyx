...
def create_static_graph_type_table(df, nombre_institucion, nombre_titulacion) -> BytesIO:
    ...
    df_agrupado = df_agrupado.sort_values(['Tipo', 'Tasa'])
    ...
    for i, tasa in enumerate(tasas_unicas, 1):
        ...
        fig.add_trace(
            go.Table(
                ...
                cells=dict(
                    values=[df_tasa['Tipo'], df_tasa['Valor']],
                    fill_color='white',
                    align=['center', 'center'],
    ...
    fig.update_layout(
        title=f"<b>Datos por Tipología: {nombre_institucion}</b><br><sub>Titulación: {nombre_titulacion}</sub>",
