...
from functions.write_presentation.constants import (
    MARGIN_L, MARGIN_T, CONTENT_W, SLIDE_W,
)
from functions.write_presentation.helpers import (
    add_text_box, add_image, remove_placeholders,
)
...
def resume_slide(presentacion: Presentation, title: str, img_path: str | None, text: str | None):
    ...
    if img_path:
        add_image(slide, img_path, img_left, top_img, img_w)

    if text:
        add_text_box(slide, text, left=MARGIN_L, top=Inches(5.0), width=CONTENT_W, height=Inches(2.3), size=12)